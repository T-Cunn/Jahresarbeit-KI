...
# training variables
w1 = tf.Variable(tf.random.uniform([1, 1], minval=-1, maxval=1), name="w1")
b1 = tf.Variable(tf.random.uniform([1], minval=-1, maxval=1), name="b1")
w2 = tf.Variable(tf.random.uniform([4, 4], minval=-1, maxval=1), name="w2")
...
#def animate(self):
 #global epoch
  ...
  for i in range(total_batch):
    ...
    gradients = tape.gradient(loss, [w2, b2])
    #optimise gradients
    optimizer.apply_gradients(zip(gradients, [w2, b2]))
    # calculate loss
