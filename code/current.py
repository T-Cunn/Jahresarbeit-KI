...
for cat in categorys:
  ...
  for img in imglist:
    ...
    dataUnsort.append([np.ndarray.tolist(img_array), i])
    if len(temp) >= sequence_length:
      dataSort.append([temp[:], i])
      del temp[0]
  i+=1
#group unsorted data
random.shuffle(dataUnsort)
tempB = []
for img, label in dataUnsort:
    temp.append(img)
    if len(temp) >= sequence_length:
      tempB.append([temp[:], label])
      del temp[0]
dataUnsort = tempB
#make training data
data = dataUnsort + dataSort
random.shuffle(data)
x_train = []
y_train = []
